...
import com.example.dripdrip.other.Constants.MAX_USERNAME_LENGTH
import com.example.dripdrip.other.Constants.MIN_PASSWORD_LENGTH
...
class AuthViewModel @ViewModelInject constructor(
    ...
    private val _resetStatus = MutableLiveData<Event<Resource<Void>>>()
    val resetStatus: LiveData<Event<Resource<Void>>> = _resetStatus
    ...
    fun reset(email: String){
        if(email.isEmpty()){
            ...
            _resetStatus.postValue(Event(Resource.Error(error)))
        } else {
            _resetStatus.postValue(Event(Resource.Loading()))
            viewModelScope.launch (dispatcher){
                val result = repository.reset(email)
               _resetStatus.postValue(Event(result))
            }
        }
    }
